...
const ALLOWED_REDIRECT_URIS = [
  ...
  'https://prediction-engine.xyz/auth/valyu/callback',
  'https://www.prediction-engine.xyz/auth/valyu/callback',
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (grant_type === 'refresh_token') {
      if (!refresh_token) {
        return NextResponse.json(
          { error: 'missing_parameters', error_description: 'refresh_token is required' },
          { status: 400 }
        );
      }
      ...
      // Handle authorization code grant (default)
