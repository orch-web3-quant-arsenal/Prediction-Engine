...
export default function ShareModal({ open, onOpenChange, marketTitle, verdict, confidence }: ShareModalProps) {
  ...
  const shareUrl = `https://prediction-engine.ai?via=share&r=${Math.random().toString(36).substr(2, 9)}`;

  const xText = `I ran this prediction market through Prediction-Engine. Verdict: ${verdict === "YES" ? "✅" : "❌"} ${verdict} (${confidence}% confidence).
...
AI-powered deep research + analyst-grade report in 5s. Try it: ${shareUrl}`;
  ...
  const redditBody = `Just analyzed this prediction market on Prediction-Engine and got a ${confidence}% confidence ${verdict} verdict.
