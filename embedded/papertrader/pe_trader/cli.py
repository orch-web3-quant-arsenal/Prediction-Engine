"""Click CLI for pe-trader — Polymarket paper trading simulator."""
...
from pe_trader.engine import Engine
from pe_trader.models import SimError

DEFAULT_DATA_DIR = Path.home() / ".pe-trader"
...
def stats(ctx: click.Context, card: bool, plain: bool, tweet: bool) -> None:
    ...
    from pe_trader.analytics import compute_stats
    ...
    try:
        ...
        if tweet or card or plain:
            from pe_trader.card import generate_card, generate_card_plain, generate_tweet
...
def pk(ctx: click.Context, account_a: str, account_b: str) -> None:
    ...
    from pe_trader.analytics import compute_stats
    from pe_trader.card import generate_pk_card

    base = ctx.obj.get("data_dir") or Path.home() / ".pe-trader"
...
def export_trades(ctx: click.Context, fmt: str, output_file: Path | None, limit: int) -> None:
    ...
    from pe_trader.export import export_trades_csv, export_trades_json
...
def export_positions(ctx: click.Context, fmt: str, output_file: Path | None) -> None:
    ...
    from pe_trader.export import export_positions_csv, export_positions_json
...
def benchmark_run(ctx: click.Context, strategy_path: str, balance: float) -> None:
    """Run a strategy: pe-trader benchmark run module.function"""
    from pe_trader.benchmark import run_strategy
...
def benchmark_pk(
    ...
    from pe_trader.benchmark import pk_battle
...
def benchmark_compare(ctx: click.Context, account_names: tuple[str, ...]) -> None:
    ...
    from pe_trader.benchmark import compare_accounts
...
def orders_place(
    ...
    """Place a limit order: pe-trader orders place SLUG yes buy 100 0.55"""
    engine = _get_engine(ctx)
    try:
...
def mcp() -> None:
    ...
    from pe_trader.mcp_server import main as mcp_main
    mcp_main()
