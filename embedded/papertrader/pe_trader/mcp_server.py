"""MCP server exposing pe-trader as tools for AI agents.

Run with:
    pe-trader mcp                  # stdio transport (default)
    python -m pe_trader.mcp_server # direct execution
"""
...
DEFAULT_DATA_DIR = Path.home() / ".pe-trader" / "default"

mcp = FastMCP("pe-trader", json_response=True)
...
def _get_engine(account: str = "default") -> Engine:
    ...
    data_dir = Path.home() / ".pe-trader" / account
...
def _ok(data: object) -> str:
    """Wrap data in the standard pe-trader JSON envelope."""
...
def leaderboard_card(accounts: str = "") -> str:
    ...
    Otherwise scans all accounts in ~/.pe-trader/.
    """
    try:
        from pe_trader.analytics import compute_stats
        from pe_trader.card import generate_leaderboard_card

        base = Path.home() / ".pe-trader"
        if accounts:
...
def pk_battle(
    ...
    """
    try:
        from pe_trader.benchmark import pk_battle as _pk_battle
...
def backtest(
    ...
    try:
        ...
        from pe_trader.backtest import (
            load_snapshots_csv,
            load_snapshots_json,
            run_backtest,
        )
        from pe_trader.benchmark import _validate_strategy_path
