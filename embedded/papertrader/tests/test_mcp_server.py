...
from pe_trader import mcp_server
from pe_trader.mcp_server import (
    backtest,
    buy,
    cancel_all_orders,
    cancel_order,
...
class TestErrFrom:
    def test_sim_error_passes_through(self):
        from pe_trader.models import InsufficientBalanceError
        from pe_trader.mcp_server import _err_from
...
class TestLeaderboardEntry:
    ...
    def test_with_trades(self):
        init_account()
        from pe_trader.mcp_server import _get_engine
...
class TestLeaderboardCard:
    ...
    def test_scan_all_accounts(self):
        # Create accounts so they exist at ~/.pe-trader/<name>
    ...
    def test_outer_exception(self):
        with patch("pe_trader.mcp_server.Path.home", side_effect=RuntimeError("boom")):
            result = _parse(leaderboard_card())
        assert result["ok"] is False
        assert result["code"] == "internal_error"
...
class TestGetMarket:
    ...
    def test_not_found(self):
        ...
        from pe_trader.mcp_server import _get_engine
        engine = _get_engine()
        from pe_trader.models import MarketNotFoundError
...
class TestBacktestTool:
    def test_invalid_strategy_path(self):
        ...
        from pe_trader.mcp_server import backtest
